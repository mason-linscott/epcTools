...
simEPC.phenogram<-function(cache,n) {
  ...
  for (i in 1:n) {
    simdat <- PCMBase::PCMSim(cache$PCMtree, model=cache$pcmModel, X0=cache$pcmModel$X0[1])
    phytools::phenogram(cache$PCMtree, simdat[1,], ftype="off",color=mycol,add=TRUE)
    #lines(base::seq(0,100, length.out=11), log(2)/pars$alpha, lty=2, col="red")
  }
  #lines(base::seq(0,cache$max_time, length.out=cache$n_slice), log(2)/cache$pars$alpha, lty=2, col="red")
}
